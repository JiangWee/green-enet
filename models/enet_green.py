...
class UpsamplingBottleneck(nn.Module):
    ...
    def __init__(self,
        ...
        # replace MaxUnpool2d (not ONNX-friendly) with Upsample(nearest)
    ...
    def forward(self, x, max_indices, output_size):
        ...
        # max_indices / output_size are ignored; Upsample used instead
        main = self.main_unpool1(main)

        # Extension branch
        ext = self.ext_conv1(x)
        # call ConvTranspose2d without output_size for ONNX friendliness
...
class ENetGreenRatio(nn.Module):
    """Generate the ENet model with green ratio estimation.
    ...
    - encoder_only (bool, optional): When ``True``, only the encoder is used
    for green ratio estimation; otherwise, full segmentation is performed.
    Default: False.

    """

    def __init__(self, num_classes, encoder_relu=False, decoder_relu=True, encoder_only=False):
        super().__init__()
